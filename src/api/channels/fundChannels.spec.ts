...
describe('test fundChannels', function () {
  ...
  it('throws a custom error when hoprd api response is an 400 error', async function () {
    nock(BASE_PATH).post('/api/v2/fundmulti').reply(400, {
      status: 'INVALID_PEERID'
    });

    await expect(
      fundChannels(BASE_PATH, API_TOKEN, {
        peerId: '16Uiu2HAmUsJwbECMroQUC29LQZZWsYpYZx1oaM1H9DBoZHLkYn12',
        outgoingAmount: '1000000',
        incomingAmount: '1000000'
      })
  ...
  it('throws a custom error when hoprd api response is an 403 error', async function () {
    nock(BASE_PATH).post('/api/v2/fundmulti').reply(403, {
  ...
  it('throws a custom error when hoprd api response is an 422 error', async function () {
    nock(BASE_PATH).post('/api/v2/fundmulti').reply(422, {
