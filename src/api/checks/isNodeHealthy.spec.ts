...
describe('test pingPeer', function () {
  ...
  it('throws ZodError when error-path body matches neither response schema nor ApiErrorResponse', async function () {
    nock(API_ENDPOINT).get(`/healthyz`).reply(400, { unexpected: 'shape' });

    let caught: any;
    try {
      await isNodeHealthy({ apiToken: API_TOKEN, apiEndpoint: API_ENDPOINT });
    } catch (e) {
      caught = e;
    }
  ...
  it('rejects when the connection errors', async function () {
    nock(API_ENDPOINT).get(`/healthyz`).replyWithError('ECONNREFUSED');
  ...
  it('rejects when error-path body is malformed JSON', async function () {
    nock(API_ENDPOINT).get(`/healthyz`).reply(400, 'not-json');
  ...
  it('throws Error when the api responds with a 500', async function () {
    nock(API_ENDPOINT).get(`/healthyz`).reply(500, 'Internal Server Error');
