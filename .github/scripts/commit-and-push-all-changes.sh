...
if [ -n "$(git diff --name-only origin/docs -- docs/)" ]; then
    ...
    git pull origin docs --rebase --strategy-option recursive -X theirs # NB! when pull rebasing, ours is the incoming change (see https://stackoverflow.com/a/3443225)
