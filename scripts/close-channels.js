...
async function closeChannel(channel) {
  ...
  try {
    const res = await api.closeChannel({
...
async function run() {
  ...
  if (closableChannels.length > 0) {
    console.log(
      `Closing ${closableChannels.length} incoming and outgoing channels`
    );
    const resMapping = closableChannels.map((ch) => ({
      ch,
      pRes: closeChannel(ch)
    }));
